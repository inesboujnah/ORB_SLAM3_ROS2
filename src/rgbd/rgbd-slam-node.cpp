...
RgbdSlamNode::~RgbdSlamNode()
{
    ...
    std::string kf_tum = "/root/memory_register/orbslam_data/trajectory/rgbd_kf_tum_" + std::string(buffer) + ".txt";
    m_SLAM->SaveKeyFrameTrajectoryTUM(kf_tum);

    std::string full_tum = "/root/memory_register/orbslam_data/trajectory/rgbd_full_tum_" + std::string(buffer) + ".txt";
    m_SLAM->SaveTrajectoryTUM(full_tum);

    // Save EuRoC keyframe and full trajectory
    std::string kf_euroc = "/root/memory_register/orbslam_data/trajectory/rgbd_kf_euroc_" + std::string(buffer) + ".txt";
    m_SLAM->SaveKeyFrameTrajectoryEuRoC(kf_euroc);

    std::string full_euroc = "/root/memory_register/orbslam_data/trajectory/rgbd_full_euroc_" + std::string(buffer) + ".txt";
